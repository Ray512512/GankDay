...
//        setTopRightButton(R.mipmap.ic_launcher_round, () -> T.makeText(mContext, "123", T.LENGTH_SHORT).show());
    ...
    protected void initEvents() {
        tvTest.setOnLongClickListener(v -> {
            ClipData results = ClipData.newPlainText("", "");
            v.startDrag(results, new View.DragShadowBuilder(v), null, 0);
            return false;
        });
...
//                DemoUser s=mUserDao.create((long) 2);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent results) {
        ...
        switch (resultCode){
            case RESULT_OK ://选择照片
                String path = "file://" + results.getStringArrayListExtra(MultiImageSelector.EXTRA_RESULT).get(0);
                switch (requestCode) {
                    case 1:
